...
class TestGC:
    ...
    def main(self, feq):
        rate = rospy.Rate(feq)
        # desired_js = np.array([.0105, .0534, 1.178, 0, 0, 0]) # pos 1
        # desired_js1 = np.array([-0.85, -0.55, 0.20, 0, 0, 0]) # pos 2
        # desired_js = np.array([0, 0, 0.12, 0, 0, 0])
        # desired_js = np.array([-1.60, 0.57, 0.10, 0, 0, 0]) # pos 3
        # desired_js = np.array([0.80, 0.58, 0.09, 0, 0, 0]) # pos 4
        # desired_js = np.array([2.00, -0.34, 0.21, 0, 0, 0]) # pos 5
        # desired_js_origin = np.array([ 0.06945898, -0.02554529,  0.16513405, -0.0,  0.000,  0.00])
        # desired_js_origin_closer = np.array([ 0.04316992, -0.03476632,  0.19538073, -0.0, -0.0,   -0.0])
        # desired_js_light_tap = np.array([ 0.04214368, -0.03300044,  0.19797412, -0.0,  0.000,  0.00]) # light tap
        # desired_js_medium_tap = np.array([ 0.04214368, -0.03300044,  0.19997412, -0.0,  0.000,  0.00]) # medium tap  
        # desired_js_hard_tap = np.array([ 0.04214368, -0.03300044,  0.200, -0.0,  0.000,  0.00]) # hard tap  

        # for j in range(3):
        #     for i in range(3):
        #         arm.move_jp(desired_js_origin).wait()
        #         arm.move_jp(desired_js_light_tap).wait()
        #         arm.move_jp(desired_js_origin).wait()
        #         arm.move_jp(desired_js_medium_tap).wait()
        #         arm.move_jp(desired_js_origin).wait()
        #         arm.move_jp(desired_js_hard_tap).wait()
        #         arm.move_jp(desired_js_origin).wait()
        #     for i in range(3):
        #         arm.move_jp(desired_js_origin).wait()
        #         arm.move_jp(desired_js_light_tap).wait()
        ...
        #     for i in range(10):
        #         arm.move_jp(desired_js_origin_closer).wait()
        #         arm.move_jp(desired_js_medium_tap).wait()
        #         # rospy.sleep(.2)
        ...
        while(1):
            ...
            a = arm.measured_jp()
            # a = arm.measured_cp()
            # print("np.array([", a[0], ",", a[1], ",", a[2], ",", a[3], ",", a[4], ",", a[5], "])")
            print("np.array([", a[0], ",", a[1], ",", a[2], ",", 0.0, ",", 0.0, ",", 0.0, "])")
            # print(a)
